...
def skew(name,t1,t2,t3,t4,t5):
    Min = min(t1,t2,t3,t4,t5)
    Max = max(t1,t2,t3,t4,t5)
    avg = (t1+t2+t3+t4+t5)/5
    var = (t1-avg)**2 + (t2-avg)**2 + (t3-avg)**2 + (t4-avg)**2 + (t5-avg)**2 
    var /= 5
    skew = (t1-avg)**3 + (t2-avg)**3 + (t3-avg)**3 + (t4-avg)**3 + (t5-avg)**3
    skew /= 5
    skew = round(skew/var**3**0.5,2)
    print("{0}'s stats-- min: {1:d}, max: {2:d}, avg: {3:.1f}".format(name,Min,Max,avg))

print(skew("Stan",34,34,35,31,29))
print(skew("Kyle",30,31,29,29,28))
print(skew("Cartman",36,31,32,33,33))
print(skew("Kenny",33,32,34,31,35))
print(skew("Bebe",27,29,29,28,30))
#name_1 = "Stan"
