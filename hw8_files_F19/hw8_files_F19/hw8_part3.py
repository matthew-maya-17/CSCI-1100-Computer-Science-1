...
for x in range(len(rb)):
    rbears.append(Bear.Bear(rb[x][0],rb[x][1],rb[x][2]))

#reserve tourist list
rtourists = []
for x in range(len(rt)):
    rtourists.append(Tourist.Tourist(rt[x][0],rt[x][1]))
...
while True:
    ...
    for b in range(len(bears)):
         for t in range(len((tourists))):
             if  bears[b].r == tourists[t].r  and bears[b].c == tourists[t].c:
                 #if so they die and bears kill counter goes up
                 tourists[t].die = True
                 bears[b].kill = True
    ...
    turn += 1
    #if there are no more bears and none lef tin queue simulation ends
    activebears = len(bears)
    if activebears == 0 and len(rbears)== 0:
        break
#prints final update
print("Field has {} berries.".format(berryfield.totalb()))
print(berryfield)
